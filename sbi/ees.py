...
class Simulator(object):
    ...
    def __init__(self, noisy: bool = True, **kwargs):
        ...
        # Constants
        default = {
            'D_pl': 9.9 * prt.nat_cst.pc, #7.34, 9.9
            'pressure_scaling': 10,
            'pressure_simple': 100,
            'pressure_width': 3,
            'scale': 1e5, #1e16,
...
#             cloud_species=['MgSiO3(c)_cd', 'Fe(c)_cd'],
            ...
            wlen_bords_micron=[2300, 2390], #[0.8, 1.8]-HST what I used, #[4.80, 18.22]-actual [4.9, 19]- what i used
            mode = 'lbl',
        )
...
#         self.atmosphere.setup_opa_structure(np.logspace(-6, 3, levels))
        self.atmosphere.setup_opa_structure(np.logspace(-6, 3, 100))
        ...
        # Noise
        self.noisy = noisy
#         self.sigmaM = np.array(obs.iloc[:, 2]) #obs[:, 2] * self.scale #1.25754e-17 * self.scale
        self.sigma = np.array(obs.iloc[:, 2]) #obs[:, 2] * self.scale #1.25754e-17 * self.scale

        # self.sigma = np.array(obs.iloc[:, 2])*(11.5/9.9)**2
        # self.sigma = np.array(obs.iloc[:, 2])*(14/9.9)**2
        # self.sigma = np.array(obs.iloc[:, 2])*(2.28/9.9)**2
        

#         w_hst = obs_hst.iloc[:, 0]
#         self.sigmaH = np.array((obs_hst.iloc[:, 2]*1e30 * (w_hst*1e-4)**2/nc.c).values)
#         self.sigma = np.append(self.sigmaH, self.sigmaM)
        # print(self.sigma)
