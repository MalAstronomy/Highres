...
@job(array=16, cpus=2, gpus=1, ram='64GB', time='10-00:00:00')
def experiment(index: int) -> None:
    ...
    def noisy(x, b= None): #50 is 10% of the median of the means of spectra in the training set.
        ...
        x = x + torch.from_numpy(data_uncertainty).cuda() * torch.randn_like(x) * b.cuda()
...
# #     ## Corner    
    x_star, _ =  noisy(torch.Tensor(np.loadtxt('x_sim_b.npy'))[0].cuda())
